...
export class NucleixCdkStack extends Stack {
  constructor(scope: Construct, id: string, props?: StackProps) {
    ...
    const cfnMicrosoftAD = new directoryservice.CfnMicrosoftAD(
      this,
      "MicrosoftAD",
      {
        name: "nucleix.com",
    ...
    // DHCP Options
    const cfnDHCPOptions = new ec2.CfnDHCPOptions(
      this,
      "DHCPOptions",
      /* all optional props */ {
        domainName: adDomain,
    ...
    const cfnDHCPOptionsAssociation = new ec2.CfnVPCDHCPOptionsAssociation(
      this,
    ...
    const filesystemBam = new fsx.CfnFileSystem(this, "FSxWindowsFilesystem", {
      ...
      windowsConfiguration: {
        ...
        preferredSubnetId: vpc.selectSubnets({
    ...
    const param = new ssm.StringParameter(this, "Parameter", {
      allowedPattern: ".*",
      description: "DnsName FSX",
      parameterName: "DnsName",
      stringValue: filesystemBam.attrDnsName,
    ...
    // S3 Source Location
    const s3SourceLocation = new datasync.CfnLocationS3(
      this,
      "S3SourceLocation",
      {
    ...
    const s3DestinationLocation = new datasync.CfnLocationS3(
      this,
    ...
    // FSx location
    const fsxLocation = new datasync.CfnLocationFSxWindows(
      this,
      "FSxLocation",
      {
        fsxFilesystemArn: `arn:aws:fsx:${region}:${accountId}:file-system/${filesystemBam.ref}`,
        securityGroupArns: [
          `arn:aws:ec2:${region}:${accountId}:security-group/${clientSG.securityGroupId}`,
        ],
    ...
    linksbucket.addEventNotification(
      s3.EventType.OBJECT_CREATED_PUT,
      new cdk.aws_s3_notifications.LambdaDestination(lambdaTriggerDatasync),
      { prefix: "input/" }
    );

    // const s3PutEventSource = new lambdaEventSources.S3EventSource(linksbucket, {
    //   events: [
    //     s3.EventType.OBJECT_CREATED_PUT
    //   ]
    // });

    // lambdaTriggerDatasync.addEventSource(s3PutEventSource);
    ...
    lambdaTriggerDatasync.addToRolePolicy(
    ...
    lambdaTriggerDatasync.addToRolePolicy(
      new iam.PolicyStatement({
        ...
        resources: [
          `arn:aws:s3:::${linksbucket.bucketName}`,
          `arn:aws:s3:::${linksbucket.bucketName}/*`,
        ],
      })
    );
    ...
    param.node.addDependency(filesystemBam);
  }
}
